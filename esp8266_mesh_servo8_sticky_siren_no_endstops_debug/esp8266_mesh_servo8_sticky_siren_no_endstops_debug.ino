/*
  ...
  - NO_ENDSTOPS Directive:
    * When #define NO_ENDSTOPS is defined, all endstop/limit switch initialization,
      pin reads, local directional safety clamping, and limit change detection logic
      are cleanly bypassed and compiled out.
  ...
  - Directional limit switch safety clamping (disabled when NO_ENDSTOPS active).
  - Optional Connection Quality Stats:
    * When #define DEBUG_CONNECTION is defined, RSSI and painlessMesh network metrics
      are attached directly to the heartbeat debug output.
*/
...
void setup() {
...
#endif
#ifndef NO_ENDSTOPS
    Serial.printf("Min Limit Pin: GPIO %d  | Max Limit Pin: GPIO %d  | Reset Alarm Pin: GPIO %d \n",
                  MIN_LIMIT_PIN, MAX_LIMIT_PIN, RESET_ALARM_PIN);
#else
    Serial.printf("Reset Alarm Pin: GPIO %d | Endstops: DISABLED (NO_ENDSTOPS defined)\n", RESET_ALARM_PIN);
#endif
...
#endif
#ifdef NO_ENDSTOPS
    Serial.println("Option: NO_ENDSTOPS ENABLED (Endstop inputs & safety limit clamping disabled)");
#endif
#ifdef DEBUG_CONNECTION
    Serial.println("Option: DEBUG_CONNECTION ENABLED (Heartbeat logs display RSSI & painlessMesh stats)");
#endif
    Serial.println("==================================================");
    ...
    delay(500); // wait for the mesh to start up
    
    // Set maximum output power (20.5 dBm = ~112mW)
    WiFi.setOutputPower(20.5);

    // Force 802.11b protocol for maximum range and penetration
    struct ip_info info;
    wifi_set_phy_mode(PHY_MODE_11B);

    // Disable Wi-Fi Sleep Mode to prevent RF power down / latency drops
 ...
 * Updates local safety clamping immediately without waiting for network timers (if endstops enabled).
...
void checkAndTransmitInputs() {
    ...
    if (stateChanged || heartbeatElapsed) {
        ...
        const char* txTypeStr = heartbeatElapsed ? "HEARTBEAT" : "EVENT";
        Serial.printf("[TX %s #%u] Filtered ADC: %.2f | Target Pulse: %.2f us (FP4: %u) | Unicast Queued: %s (CRC: 0x%04X)",
                      txTypeStr, msg.seq, filteredAdc, (float)currentUsFp4 / 16.0f, currentUsFp4,
                      sentDirect ? "SUCCESS" : "FAILED", msg.crc16);

#ifdef DEBUG_CONNECTION
        int8_t rssi = WiFi.RSSI();
        SimpleList<uint32_t> nodeList = mesh.getNodeList();
        size_t totalMeshNodes = nodeList.size();
        uint32_t myMeshId = mesh.getNodeId();

        Serial.printf(" | [CONN DEBUG] RSSI: %d dBm | Peer MeshID: %u | My MeshID: %u | Total Nodes: %zu",
                      rssi, peerSession.meshNodeId, myMeshId, totalMeshNodes);
#endif
...
#else
    Serial.printf("[SERVO FP4] Requested: %.2f us -> Applied: %u us (%u FP4) | LastSafe: %u FP4\n",
                  (float)requestedServoUsFp4 / 16.0f, targetUsInt, appliedServoUsFp4, lastSafeUsFp4);
#endif
}
